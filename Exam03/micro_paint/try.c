...
{
    if (x < rect->x || y < rect->y || rect->x + rect->w < x || rect->h + rect->y < y)
        return 0;
    if (x - rect->x < 1.00000000 || y - rect->y < 1.00000000 || rect->w + rect->x - x < 1.00000000 || rect->h + rect->y - y < 1.00000000)
        return 2;
    return 1;
}
    ...
    while ((ret = fscanf(file, " %c %f %f %f %f %c\n", &rect.type, &rect.x, &rect.y, &rect.w, &rect.h, &rect.c)) == 6)
    {
        ...
        while (j < zone->height)
        {
            ...
            while (i < zone->width)
            {
                ...
                if ((ret1 == 2 && rect.type == 'r') || (ret1 && rect.type == 'R'))
